...
public class AngleUtil {
	...
	public static final double PI   = Math.PI;    // PI   :: 180deg
	...
	public static Point getTargetPoint(Point position, double distance, double heading) {
		...
		double targetX = position.getX() + (distance * Math.cos(heading));
		double targetY = position.getY() + (distance * Math.sin(heading));
